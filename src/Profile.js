    ...
    return (
        <form className='Profile'
            onSubmit={event => {
                ...
                const myImage = document.getElementById('profilePicture')
                ...
                event.target.newPicture.files[0] ?
                    this.setState({resizingPicture: true, myImageFile: event.target.newPicture.files[0]}) :
                    props.updateUser({
                        _id: props.data.user._id,
                        name: props.data.user.name,
                        newUsername,
        ...
        >
            ...
            <div className='changePicture'>
                <label>
                    ...
                    {/* <img id='myImage' src={this.state.myImageDataURL} alt='uploaded profile'/></> */}
                    <img id='profilePicture' 
                    src={this.state.file ? 
                        blobUrl(this.state.file) : 
                        this.props.data.user.pictureSrc} 
                    alt={`user's profile`}/>
                    {/* } */}
                    <input 
                        type='file' name='newPicture' accept='image/*' 
                        onChange={event => this.setState({pictureUploaded: true, myImage: event.target.files[0]})}
                        onFocus={() => {
                            this.setState({
                                usernameSearchTerm: '',
                                newPassword: ''
